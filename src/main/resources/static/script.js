...
async function initMap() {
    ...
    map.addListener('click', function (event) {
        if (i < 2) {
            ...
            if(i < 1){
                ...
                console.log(`Miejsce: Latitude ${event.latLng.lat()}`)
                console.log(`Miejsce: Longitude ${event.latLng.lng()}`)
                startPlace = {
            ...
            markers[markerKey].addListener('dragend', function () {
                ...
                startPlace.latitude = position.lat;
                startPlace.longitude = position.lng;
                console.log(`${markers[markerKey].title} został przeniesiony na: ${position.lat}, ${position.lng}`);
                if (markerKey === 'marker1') {
                    document.getElementById('origin').value = position;
                }
                if (markerKey === 'marker2') {
                    document.getElementById('destination').value = position;
                }
            });
            i += 1;
        }
    ...
    document.getElementById("origAndDestPoints").addEventListener("click", function () {
        let routeId;
        if (markers.marker1 && markers.marker2) {
            routeId = addRouteToDB(markers.marker1.position, markers.marker2.position);
        } else {
            alert("Proszę wybrać dwa punkty na mapie.");
        }
...
document.getElementById("entertainmentAndRecreation").addEventListener('click', () => handleButtonClick('entertainment', startPlace));
document.getElementById("foodAndDrink").addEventListener('click', () => handleButtonClick('foodAndDrink', startPlace));
document.getElementById("culture").addEventListener('click', () => handleButtonClick('culture', startPlace));
document.getElementById("sport").addEventListener('click', () => handleButtonClick('sport', startPlace));
document.getElementById("busStop").addEventListener('click', () => handleButtonClick('busStop', startPlace));
}
...
// function choiceService(origin){
//     // sprawdzenie który checkbox jest zaznaczony
//     let type = typeChoice();
//
//     // wyciągnięcie punktu z markera origin (ewentualnie bazy danych) i przekazanie do places api punktu i typu miejsca
//     let places = searchForPlaces(origin,type)
//
//     // otrzymanie punktów i zaznaczenie ich na mapie z funkcją listener
//     let placeMarkers = addMarkers(places);
//
//     // sprawdzenie który został kliknięty i zapisanie punktu jako nastepny punkt odniesienia oraz w bazie danych
//
//     // usunięcie pozostałych punktów z mapy oraz wyczyszczenie checkboxów
// }